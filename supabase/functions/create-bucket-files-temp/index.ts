...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.*", "text/*"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for files" ON storage.objects FOR SELECT USING (bucket_id = 'files');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for files" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'files');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for files" ON storage.objects FOR UPDATE USING (bucket_id = 'files');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for files" ON storage.objects FOR DELETE USING (bucket_id = 'files');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'files',
        public: true,
        allowed_mime_types: ["application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.*", "text/*"],
